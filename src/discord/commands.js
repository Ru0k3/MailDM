...
export async function handleInteraction(interaction, deps) {
  ...
  if (name === 'summary-now') {
    ...
    try { const result = await runSummaryForUser({ discordUserId, store: deps.store, env: deps.env, fetchImpl: deps.fetchImpl, gmailAdapterFactory: deps.gmailAdapterFactory, summarizerFactory: deps.summarizerFactory, autoRecord: true }); return reply(result.summary, { components: feedbackComponents }); }
