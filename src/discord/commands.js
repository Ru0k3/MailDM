import { encryptSecret } from '../security/index.js';
...
export async function handleInteraction(interaction, deps) {
  ...
  if (name === 'disconnect') {
    await deps.store.disconnectAndPurge(discordUserId, option(interaction, 'email'));
  ...
  if (name === 'delete-my-data') {
    await deps.store.deleteAllUserData(discordUserId);
    return reply('Deleted all MailDM data for this Discord user, including Gmail tokens, settings, feedback, and summary history.');
  }
  if (name === 'settings') {
  ...
  if (name === 'set-time') {
    ...
    const timezone = option(interaction, 'timezone') ?? (await deps.store.getSettings(discordUserId)).timezone;
    ...
    await deps.store.updateSettings(discordUserId, { summaryTime: time, timezone });
  ...
  if (name === 'set-ai-provider') {
    ...
    await deps.store.updateSettings(discordUserId, { aiProvider: provider });
  ...
  if (name === 'set-model') {
    ...
    await deps.store.updateSettings(discordUserId, { aiModel: model });
  ...
  if (name === 'set-ai-key') {
    ...
    await deps.store.updateSettings(discordUserId, { aiApiKey: encryptSecret(key, deps.env.SESSION_SECRET) });
