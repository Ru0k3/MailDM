...
export async function makeMysqlStore(connectionString = process.env.DATABASE_URL) {
  ...
  return {
    ...
    async disconnectAndPurge(discordUserId, email) { const user = await this.getUser(discordUserId); if (!user) return; const connection = await pool.getConnection(); try { await connection.beginTransaction(); if (email) await connection.query('DELETE FROM gmail_accounts WHERE user_id=? AND email=?', [user.id, email]); else await connection.query('DELETE FROM gmail_accounts WHERE user_id=?', [user.id]); const [remaining] = await connection.query('SELECT COUNT(*) AS count FROM gmail_accounts WHERE user_id=?', [user.id]); if (!email || Number(remaining[0].count) === 0) await connection.query('DELETE FROM users WHERE id=?', [user.id]); await connection.commit(); } catch (error) { await connection.rollback(); throw error; } finally { connection.release(); } },
