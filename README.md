...
The default due window is ten minutes (`SCHEDULER_DUE_WINDOW_MINUTES=10`). The external scheduler should run every five minutes. If a cold start or temporary delay means a request arrives several minutes after the user’s configured local time, the user is still considered due during the window. Every scheduled attempt first claims `(user_id, local_date, delivery_kind='scheduled')` in `summary_history`; the SQLite `UNIQUE` constraint means a retry or an overlapping external request cannot deliver two briefs for the same user and local calendar date. A failed job remains recorded as `failed`, so the claim protects against duplicate deliveries even when a later request retries the same window.
