...
A failed claim may be retried once only when the failure occurred before delivery was attempted and the row has `attempt_count=1`; reauthorization and no-account failures are not retried. Immediately before calling Discord, the row is marked `delivery_attempted=1`. Any delivery-side failure is permanently non-retryable for that local date because the request may have reached Discord. This prevents duplicate delivery when a response is lost after a message may have been accepted.

For production Autoscale, the database behind this claim must be durable and shared across instances. The current `better-sqlite3`/`DATABASE_PATH` implementation is safe for a single process with persistent storage, but a local SQLite file is not sufficient for multi-instance or ephemeral Autoscale storage. Use a durable shared database adapter, Reserved hosting with a persistent volume, or another single-process host before production scheduling.
